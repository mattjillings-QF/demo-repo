...
function calculatePages(slider) {
    ...
    for (let startIndex = 0; startIndex < totalSlides; startIndex += slidesToScroll) {
        ...
        if (overflow) {
            // if not looped
            if (!loop) {
                // adjust the startIndex (clamp so slidesPerView > totalSlides still yields a valid page)
                const startIndex = Math.max(0, lastIndex - slidesToShow + 1);
 ...
 *
 * slidesToShow may exceed totalSlides (Swiper-style slidesPerView): slide width uses
 * --slides-to-show so fewer slides render narrower with empty track space; isStatic is then true.
 */
...
function updateTransform(slider) {
    ...
    if (offset === 0) {
        track.style.transform = `translate3d(calc(${dragged}px + (var(--blaze-loop-base-slides, 0) * (var(--slide-width) + ${slider.config.slideGap})) + var(--blaze-centered-offset, 0px)),0px,0px)`;
    }
    else {
        track.style.transform = `translate3d(  calc( ${dragged}px + ${offset} * (var(--slide-width) + ${slider.config.slideGap}) + (var(--blaze-loop-base-slides, 0) * (var(--slide-width) + ${slider.config.slideGap})) + var(--blaze-centered-offset, 0px)),0px,0px)`;
    }
}

function isCenteredLoopMode(slider) {
    const centeredMode = slider?.el?.dataset?.sliderCentered;
    if (!centeredMode || centeredMode === 'none')
        return false;
    if (!slider?.config?.loop)
        return false;
    if (centeredMode === 'both')
        return true;
    if (typeof window === 'undefined' || typeof window.matchMedia !== 'function')
        return false;
    if (centeredMode === 'mobile')
        return window.matchMedia('(max-width: 767.98px)').matches;
    if (centeredMode === 'desktop')
...
function construct(config, slider) {
    ...
    if (isCenteredLoopMode(slider) && slider.slides.length > 1) {
        wrapPrev(slider, 1);
        slider.el.style.setProperty('--blaze-loop-base-slides', '-1');
    }
    else {
        slider.el.style.setProperty('--blaze-loop-base-slides', '0');
    }
